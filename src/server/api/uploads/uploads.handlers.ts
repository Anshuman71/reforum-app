...
function buildAvatarStoragePath(userId: string, filename: string) {
  const uploadId = newId('upload');
  return `avatars/${userId}/${uploadId}-${sanitizeFilename(filename) || 'avatar'}`;
}

function buildContentImageStoragePath(userId: string, filename: string) {
  const uploadId = newId('upload');
  return `content/${userId}/${uploadId}-${sanitizeFilename(filename) || 'image'}`;
}
...
export const prepareAvatarUpload: AppRouteHandler<PrepareAvatarUploadRoute> = async c => {
  const user = assertAuthenticated(c.get('user'));
  const payload = c.req.valid('json');

  assertAvatarInput(payload.mimeType, payload.size);

  const storagePath = buildAvatarStoragePath(user.id, payload.filename);
...
export const completeAvatarUpload: AppRouteHandler<CompleteAvatarUploadRoute> = async c => {
  const user = assertAuthenticated(c.get('user'));
  const payload = c.req.valid('json');
...
> = async c => {
  ...
  const storagePath = buildContentImageStoragePath(user.id, payload.filename);
  const target = await getStorage().prepareUpload({
    filename: payload.filename,
    storagePath,
    mimeType: payload.mimeType,
